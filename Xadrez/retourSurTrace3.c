...
#define TAILLE 20
int compteur = 0;
//1. Plutot que determiner si on peut placer 4 dammes dans 4x4 imprimez toutes les façons de placer 4 dames
//2. Combien y-a-t-il de façon de places 8 dames dans un echiquier ordinaire 8
//*********************************************************
...
{
  if(nbDames == 0){
    compteur++; // incremeti antes de printar com o ++ antes.
    //printEchiquier(echiquier);
  }
  int i = (TAILLE - nbDames);
  for(int j = 0; j < TAILLE; j++){
    if(libre(echiquier, i, j)){
      echiquier [i][j] = 1; // met un damme
      if(dames(echiquier, nbDames - 1)){
        return 1;
      }
      echiquier [i][j] = 0; //enléeve la damme
    }
  }
  return 0;
}
...
//  printEchiquier(echiquier);
    printf("Nombre de solution total: %d\n",compteur);
}